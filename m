...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 3A1FD22734B
	for <lists+linux-pm@lfdr.de>; Tue, 21 Jul 2020 01:53:47 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726735AbgGTXw1 (ORCPT <rfc822;lists+linux-pm@lfdr.de>);
        Mon, 20 Jul 2020 19:52:27 -0400
Received: from mga18.intel.com ([134.134.136.126]:62265 "EHLO mga18.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726021AbgGTXu2 (ORCPT <rfc822;linux-pm@vger.kernel.org>);
        Mon, 20 Jul 2020 19:50:28 -0400
IronPort-SDR: LyP0kubldB+d1/f1noeOp9IKyl1EhcP17HQUxIyhXQB7g7aol96QesEE2oh6UytffvulqwD3e6
 26V2Ck5wtJjw==
X-IronPort-AV: E=McAfee;i="6000,8403,9688"; a="137518205"
X-IronPort-AV: E=Sophos;i="5.75,375,1589266800"; 
   d="scan'208";a="137518205"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
  by orsmga106.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 20 Jul 2020 16:50:26 -0700
IronPort-SDR: EpyQb2Ha/A0TcpZH/8bNphqkrU3dLSC2fMZdPAHns3BTQ67R5QtHoe8G/l+JjF/0EYXY3kgKx1
 7re32LXVcf/Q==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.75,375,1589266800"; 
   d="scan'208";a="487892898"
Received: from sckaira-mobl.amr.corp.intel.com (HELO technocore.fios-router.home) ([10.254.78.169])
  by fmsmga005.fm.intel.com with ESMTP; 20 Jul 2020 16:50:26 -0700
From:   Todd Brandt <todd.e.brandt@linux.intel.com>
To:     linux-pm@vger.kernel.org, rafael.j.wysocki@intel.com,
        rjw@rjwysocki.net
Cc:     todd.e.brandt@linux.intel.com, todd.e.brandt@intel.com
Subject: [PATCH] pm-graph v5.7 - important s2idle fixes
Date:   Mon, 20 Jul 2020 16:50:24 -0700
Message-Id: <98dfaf0c971eda7e3547201a37e7dc38ad0d65e8.1595286797.git.todd.e.brandt@linux.intel.com>
...
---
 ...
 tools/power/pm-graph/sleepgraph.py | 248 +++++++++++++++++------------
...
index 602e64b68ba7..2395cc2498f8 100755
...
+	machinesuspend = 'machine_suspend\[.*'
 	def __init__(self):
 		self.stamp = ''
 		self.sysinfo = ''
@@ -2812,16 +2830,13 @@ class TestProps:
...
-			return True
 			...
 			return True
@@ -2834,6 +2849,20 @@ class TestProps:
...
+			return True
 	...
 	def parseStamp(self, data, sv):
 		# global test data
@@ -2858,9 +2887,13 @@ class TestProps:
 				data.stamp[key] = val
...
+		sv.cmdline = self.cmdline
 		if not sv.stamp:
 			sv.stamp = data.stamp
 		# firmware data
@@ -3052,20 +3083,7 @@ def appendIncompleteTraceLog(testruns):
 	for line in tf:
...
+		if tp.stampInfo(line, sysvals):
 		...
 		# parse only valid lines, if this is not one move on
 		m = re.match(tp.ftrace_line_fmt, line)
@@ -3166,33 +3184,19 @@ def parseTraceLog(live=False):
...
+		if tp.stampInfo(line, sysvals):
 		...
 		if line[0] == '#':
@@ -3303,16 +3307,29 @@ def parseTraceLog(live=False):
...
@@ -3343,6 +3360,19 @@ def parseTraceLog(live=False):
...
@@ -3374,9 +3404,8 @@ def parseTraceLog(live=False):
 				if(not m):
 					continue
 				n = m.group('d')
...
@@ -3479,7 +3508,12 @@ def parseTraceLog(live=False):
...
+					data.newActionGlobal(title, event['begin'], event['end'], event['pid'])
 			...
 			for key in sorted(tp.ktemp):
 				name, pid = key
@@ -3548,8 +3582,9 @@ def parseTraceLog(live=False):
...
@@ -3611,7 +3646,7 @@ def loadKernelLog():
 		idx = line.find('[')
 		if idx > 1:
 			line = line[idx:]
-		if tp.stampInfo(line):
+		if tp.stampInfo(line, sysvals):
 		...
 		m = re.match('[ \t]*(\[ *)(?P<ktime>[0-9\.]*)(\]) (?P<msg>.*)', line)
 		if(not m):
@@ -3959,18 +3994,20 @@ def addCallgraphs(sv, hf, data):
...
+				name += ' {%s}' % dev['drv']
 			if sv.suspendmode in suspendmodename:
 				name += ' '+p
 			if('ftrace' in dev):
@@ -4517,12 +4554,9 @@ def createHTML(testruns, testfail):
...
+					drv = xtraclass = xtrainfo = xtrastyle = ''
 					...
 					if 'color' in dev:
@@ -4553,7 +4587,7 @@ def createHTML(testruns, testfail):
 						title += b
...
+						dname+drv, xtraclass, xtrastyle)
 					if('cpuexec' in dev):
 						for t in sorted(dev['cpuexec']):
 							start, end = t
@@ -4571,6 +4605,8 @@ def createHTML(testruns, testfail):
 						continue
...
@@ -5876,7 +5912,7 @@ def getArgFloat(name, args, min, max, main=True):
...
+		pprint('PROCESSING: %s' % sysvals.htmlfile)
 	...
 	error = ''
@@ -5928,7 +5964,7 @@ def processData(live=False, quiet=False):
...
@@ -5984,25 +6020,27 @@ def runTest(n=0, quiet=False):
 	return 0
...
-		n += i+j
 	if firstonly:
 		return ''
 	return out
@@ -6034,7 +6072,7 @@ def data_from_html(file, outpath, issues, fulldetail=False):
 	else:
 		result = 'pass'
 	# extract error info
-	ilist = []
+	tp, ilist = False, []
 	...
 	log = find_in_html(html, '<div id="dmesglog" style="display:none;">',
 		'</div>').strip()
@@ -6042,8 +6080,8 @@ def data_from_html(file, outpath, issues, fulldetail=False):
 		d = Data(0)
 		d.end = 999999999
...
@@ -6059,8 +6097,8 @@ def data_from_html(file, outpath, issues, fulldetail=False):
 	if wifi:
 		extra['wifi'] = wifi
...
+		issue = 'FREEZEWAKE'
 		...
 		if len(match) > 0:
 			match[0]['count'] += 1
@@ -6126,6 +6164,11 @@ def data_from_html(file, outpath, issues, fulldetail=False):
 		data[key] = extra[key]
 	if fulldetail:
...
@@ -6155,8 +6198,7 @@ def runSummary(subdir, local=True, genhtml=False):
...
@@ -6165,6 +6207,8 @@ def runSummary(subdir, local=True, genhtml=False):
...
@@ -6172,6 +6216,8 @@ def runSummary(subdir, local=True, genhtml=False):
